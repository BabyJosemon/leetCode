#this question can be thought of as a binary search question. The largest value of k can be the largest value of piles array which will take len(piles) hrs for koko to finish. eg. at speed 11 the time taken by koko will be 4 (len(piles)) but this is not necesarrily the smallest. The answer lies in the array from 1 to the largest value in piles. So we create the array, set smallest value to be the largest in this array initially.
...
class Solution:
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        largestK=max(piles)
        #possiblek = [i for i in range(1,largestK+1)]
        start=1
        end=largestK
