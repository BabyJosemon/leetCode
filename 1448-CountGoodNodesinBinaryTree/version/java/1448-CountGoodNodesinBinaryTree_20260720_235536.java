// Last updated: 20/07/2026, 23:55:36
1/**
...
28    }
29
30    private int dfs(TreeNode root, Integer maxValueTillNow) {
31        if (root == null) {
32            return 0;//no root, no good value
33        }
34        int count = 0;//my global variable
...
43    }
44}
45/**If we were asked to get the list of good nodes then that would be our global variable that we keep adding to eg.
46
47public List<TreeNode> goodNodes(TreeNode root) {
48    List<TreeNode> myList = new ArrayList<>();
49    dfs(root, Integer.MIN_VALUE, myList);
50    return myList;
51}
52private void dfs(TreeNode root, Integer maxValueTillNow, List<TreeNode> currentList){
53    if(root == null){
54        return;
55    }
56    if(root.val>=maxValueTillNow){
57        currentList.add(root);
58        maxValueTillNow = root.val;
59    }
60    dfs(root.left, maxValueTillNow, currentList);
61    dfs(root.right, maxValueTillNow, currentList);
62    myList.addAll(leftSubTree);
63    myList.addAll(rightSubTree);
64    return myList;
65}
66this adds more complexity of merging lists but if we did not have a return type to the method and 
67 */